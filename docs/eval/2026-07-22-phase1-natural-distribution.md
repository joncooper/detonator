...
**Real detection miss — malicious code demonstrably executed, we did not flag it (1):**
- `supervot` (npm): ran `node exploit.js` (twice). Its observable behavior was reading
  `/app/.git/config`, `/app/.npmrc`, `/etc/passwd` — but with **no exfil egress** (no
  unknown-domain, no non-registry socket) and no dropped executable. We score credential
  reads info-tier on purpose (npm itself reads `.npmrc` 1468× in the same trace), so a
  package-supplied script reading creds is currently indistinguishable from npm doing it.
  **Lever: attribute file reads to the package-supplied script vs. npm/pip internals**,
  and elevate when a non-package-manager process reads credentials.
...
- `bytedtrace` (pypi): `NO_PKG` — the Datadog repack found no package root (edge-case
  layout). To fix in the repack logic.
...
On a natural, un-cherry-picked slice, the detector catches ~3 in 4 of what detonates,
with a **single** case where executed malware slipped past (and a clear lever to close
it). The rest of the "misses" are packages that did nothing observable in-window, plus
two harness failures. The pypi precision fix is the headline: without the benign control
we'd have reported inflated pypi recall built on sandbox noise.
